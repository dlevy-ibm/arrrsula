  ...
  def run
    ...
    acquire_rabbitmq_queues.each do |queue|
      ...
      if queue['backing_queue_status']['avg_egress_rate'].to_f == 0
        crit_queues[queue['name']]  = "Infinite (len=#{messages})"
        next
      end

      secs_till_empty = messages / queue['backing_queue_status']['avg_egress_rate']

      # place warn / crit counts into hashes to be parsed for the alert message
      if secs_till_empty > config[:critical].to_i
        crit_queues[queue['name']] = "#{secs_till_empty} secs (len=#{messages})"
      elsif secs_till_empty > config[:warn].to_i
        warn_queues[queue['name']] = "#{secs_till_empty} secs (len=#{messages})"
      end
    end

    # decide if we need to alert and build the message
    if !crit_queues.empty?
      critical "Drain time: #{crit_queues.map { |q, c| "#{q} #{c}" }.join(', ')}"
    elsif !warn_queues.empty?
      warning "Drain time: #{warn_queues.map { |q, c| "#{q} #{c}" }.join(', ')}"
    else
